...
var _ = Describe("Buildah build conformance test", func() {
	...
	It("link test", func() {
		...
		buildah.WaitWithDefaultTimeout()
		Expect(buildah.ExitCode()).To(Equal(0))
		buildah = buildahtest.BuildAh([]string{"run", "hello-working-container", "cat", "/tmp/hello_world.sh"})
		buildah.WaitWithDefaultTimeout()
		Expect(buildah.ExitCode()).To(Equal(0))
		...
		buildah.WaitWithDefaultTimeout()
		Expect(buildah.ExitCode()).To(Equal(0))
		buildah = buildahtest.BuildAh([]string{"run", "hello-working-container-1", "cat", "/tmp/hello_world.sh"})
		buildah.WaitWithDefaultTimeout()
		Expect(buildah.ExitCode()).To(Equal(0))
