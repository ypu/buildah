...
func (p *BuildAhTest) Docker(args []string) *BuildahTestSession {
	...
	session, err := gexec.Start(command, GinkgoWriter, GinkgoWriter)
	if err != nil {
		Fail(fmt.Sprintf("unable to run buildah command: %s", strings.Join(dockerOptions, " ")))
	}
	return &BuildahTestSession{session}
}
...
func CompareJSON(a map[string]interface{}, b map[string]interface{}, skip []string) ([]string, []string, []string, bool) {
	var missKeys []string
	var leftKeys []string
	var diffKeys []string
	isSame := true
        ...
        for k, v := range a {
                ...
                case map[string]interface{}:
                        submiss, subleft, subdiff, ok := CompareJSON(v.(map[string]interface{}), vb.(map[string]interface{}), skip)
...
func InspectCompareResult(miss, left, diff []string) string {
	...
	Item have different value: DIFFKEYS`
...
// Copy file from src to dst. If the file already exist then replace it.
